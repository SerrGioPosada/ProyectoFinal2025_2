...
public class ForgotPasswordController implements Initializable {
    ...
    private void setupFloatingLabel(TextField textField, Label label) {
        ...
        textField.focusedProperty().addListener((obs, wasFocused, isNowFocused) -> {
            if (isNowFocused) {
                ...
                animateFloatingLabel(label, textField, true);
            } else if (textField.getText().isEmpty()) {
                // On blur: only hide label if field is empty
                animateFloatingLabel(label, textField, false);
            }
        ...
        textField.textProperty().addListener((obs, oldVal, newVal) -> {
            if (!newVal.isEmpty() && label.getOpacity() < 0.5) {
                ...
                animateFloatingLabel(label, textField, true);
            } else if (newVal.isEmpty() && !textField.isFocused()) {
     ...
     */
    private void animateFloatingLabel(Label label, TextField field, boolean moveUp) {
        ...
        if (moveUp) {
            ...
            translate.setToY(35);
            fade.setToValue(0.0);
            String prompt = field == txtEmail ? "EMAIL" :
                           field == txtName ? "NOMBRE COMPLETO" :
                           field == txtPassword ? "NUEVA CONTRASEÑA" :
                           "CONFIRMAR CONTRASEÑA";
            fade.setOnFinished(e -> field.setPromptText(prompt));
        }
        translate.play();
        fade.play();
    }
