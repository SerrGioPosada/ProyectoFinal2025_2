...
public class ChartUtil {
    ...
    /**
     * Populates a PieChart with data from a map and applies colors.
    ...
    public static void populatePieChart(PieChart chart, Map<String, ? extends Number> data) {
        ...
        // Apply colors using nodeProperty listener (after nodes are created)
    ...
    /**
     * Applies colors to pie chart slices using inline styles.
     * This ensures colors are visible even in dark theme.
     */
    private static void applyPieChartColors(PieChart chart) {
        ...
        for (PieChart.Data slice : chart.getData()) {
            ...
            // Use node property listener to ensure node exists
            slice.nodeProperty().addListener((observable, oldNode, newNode) -> {
                if (newNode != null) {
                    String color = colors[currentIndex % colors.length];
                    // Apply with !important equivalent by using inline style
                    newNode.setStyle("-fx-background-color: " + color + "; -fx-pie-color: " + color + ";");
                }
            });

            // Also try to apply immediately if node already exists
            if (slice.getNode() != null) {
                String color = colors[index % colors.length];
                slice.getNode().setStyle("-fx-background-color: " + color + "; -fx-pie-color: " + color + ";");
            }
