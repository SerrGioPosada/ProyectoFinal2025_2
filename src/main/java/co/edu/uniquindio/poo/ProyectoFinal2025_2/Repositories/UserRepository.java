...
public class UserRepository {
     ...
     * Since the user object is already in memory and has been modified,
     * this method simply persists the changes to the file.
     *
    ...
    public void updateUser(User user) {
        ...
        if (!usersById.containsKey(user.getId())) {
            Logger.warning("Cannot update user: User with ID " + user.getId() + " not found in repository");
            return;
        }

        Logger.info("Updating user: " + user.getEmail() + " (ID: " + user.getId() + ")");

        // Update both maps (in case email changed)
        usersById.put(user.getId(), user);
        usersByEmail.put(user.getEmail().toLowerCase(), user);

        saveToFile();
        Logger.info("User updated and saved to file");
    }
