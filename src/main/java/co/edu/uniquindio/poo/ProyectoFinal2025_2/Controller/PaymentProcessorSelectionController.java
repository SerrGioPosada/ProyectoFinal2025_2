...
public class PaymentProcessorSelectionController implements Initializable {
     ...
     * Loads the Mercado Pago payment view in the Index content area.
     */
    @FXML
    private void handleSelectMercadoPago() {
        ...
        try {
            if (indexController == null) {
                Logger.error("IndexController not set");
                co.edu.uniquindio.poo.ProyectoFinal2025_2.Util.UtilController.DialogUtil.showError(
                    "Error", "No se puede proceder con el pago en este momento");
                return;
            }
     ...
     * Loads the app's internal payment view in the Index content area.
     */
    @FXML
    ...
    /**
     * Handles "Cancelar" button click - cancels the order if possible, resets wizard to step 1 and clears all data.
     */
    @FXML
