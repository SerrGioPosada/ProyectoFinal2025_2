...
import co.edu.uniquindio.poo.proyectofiinal2025_2.Model.dto.*;
import co.edu.uniquindio.poo.proyectofiinal2025_2.Repositories.*;
...
import co.edu.uniquindio.poo.proyectofiinal2025_2.Util.UtilModel.ShipmentValidator;
import co.edu.uniquindio.poo.proyectofiinal2025_2.Util.UtilService.IdGenerationUtil;
import co.edu.uniquindio.poo.proyectofiinal2025_2.Util.Adapter.ShipmentDTOConverter;
...
public class ShipmentService {
    ...
    public ShipmentDTO updateShipment(ShipmentDTO dto) {
        ...
        // Only allow updates if status is PENDING_ASSIGNMENT
        if (shipment.getStatus() != ShipmentStatus.PENDING_ASSIGNMENT) {
            throw new IllegalArgumentException("Solo se pueden modificar envíos pendientes de asignación");
        }
    ...
    public boolean cancelShipment(String id) {
        ...
        // Can only cancel if PENDING_ASSIGNMENT
        if (shipment.getStatus() != ShipmentStatus.PENDING_ASSIGNMENT) {
            throw new IllegalArgumentException("Solo se pueden cancelar envíos pendientes de asignación");
        }
    ...
    public QuoteResultDTO quoteShipment(QuoteDTO quote) {
        ...
        Logger.info("Quote calculated using Decorator pattern - Total: $" + totalCost);

        return new QuoteResultDTO(baseCost, weightCost, volumeCost, distanceCost,
    ...
    public boolean assignDeliveryPerson(String shipmentId, String deliveryPersonId) {
        ...
        if (shipment.getStatus() != ShipmentStatus.PENDING_ASSIGNMENT) {
            throw new IllegalArgumentException("Solo se pueden asignar envíos pendientes");
        }
        ...
        changeStatus(shipmentId, ShipmentStatus.IN_TRANSIT, "Asignado a repartidor", "SYSTEM");
    ...
    public ShipmentStatsDTO getStatistics(LocalDate dateFrom, LocalDate dateTo) {
        ...
        long pending = shipments.stream().filter(s -> s.getStatus() == ShipmentStatus.PENDING_ASSIGNMENT).count();
